...
export const DeterminingPolicies = Schema.Array(
	Schema.Struct({ policyId: Schema.String }),
);
export const Errors = Schema.Array(
	Schema.Struct({ errorDescription: Schema.String }),
);
...
export const serializeAction = (ctx?: AST.AST) => {
	...
	return (action: InstanceType<Action<any, any, any, any, any>>) =>
		Effect.gen(function* () {
...
export const Action =
	...
	<
		Tag extends string,
		Principals extends Schema.Schema<any, any, IdentifiableEntity>[],
		Resources extends Schema.Schema<any, any, IdentifiableEntity>[],
		Context extends Schema.Struct.Fields = never,
	>(
		tag: Tag,
		args: {
		...
		class Cls extends TaggedRequest<Cls>(tag)(
			tag,
			{
				success: Success,
				failure: Failure,
		...
		return Cls as unknown as Action<
			Self,
			Tag,
			Principals,
			Resources,
			[Context] extends [never] ? never : Schema.Struct<Context>
		>;
	};
